package com.ariba.es.ai.hub.proxy.sessions;
...
public class SeleniumSessions {
    ...
    public static String getApiFromPath(String pathInfo) {
        Matcher matcher = API_PATTERN.matcher(pathInfo);
        if (matcher.matches()) {
            return matcher.group(1);
        }
        throw new IllegalArgumentException("Invalid request. Api is not present");
    }

    public static String getParameter(String path){
